...
Begin {
    ...
    $emailBody += '  <head>'
    $emailBody += '    <style>'
    $emailBody += '      TABLE {border-collapse: collapse;}'
    $emailBody += '      TD, TH {text-align: left; padding: 3px; border-width: 1px; border-color: #000000; border-style: dotted;}'
    $emailBody += '      TR:nth-child(even) {background-color: #f2f2f2;}'
    $emailBody += '    </style>'
    $emailBody += '  </head>'
    $emailBody += '  <body>'
    
    $MailCompose = @{
        SmtpServer = 'mail.example.edu'
    ...
    $SearchBase        = 'OU=Users,OU=SI,DC=shermco,DC=ind'
    #$disabledOU        = 'OU=DisabledUsers,DC=example,DC=edu'   ### Replace this out with the DisginguishedName of your Disabled OU
    $disabledOU        = 'OU=Disabled Accounts,OU=SI,DC=shermco,DC=ind'

    ### SamAccountNames or OUs to ignore
    $IgnoreSAM  = 'ldap|emdshop'
    $IgnoreName = 'System|Enthalpy|Dropbox|ADMT|Department|SSI, Help'
    $ignoreOU   = 'Admin|Cisco|Services'
    ...
    If ($ValidMonths -gt 1) {$ValidMonths *= -1}
    ...
    $scriptName        = $ScriptRoot.Basename                             ### Get the file name of the script without the .ps1 
    $scriptFolderName  = Split-Path -Path $ScriptRoot.DirectoryName -Leaf ### Get the name of the folder the script is in
    ...
    Function Write-ScriptLog         {
        ...
        #>
        Param (
            [Parameter(Mandatory=$true,HelpMessage='Log File')][string]$LogFile,
            [Parameter(Mandatory=$true,HelpMessage='Log Mesg')][object[]]$Content
        )
        Try {
    ...
    Function Get-GraphToken          {
        ...
        #>
        Param (
            [Parameter(Mandatory = $true,HelpMessage='Tenant Client ID')] [string]$ClientID,
            [Parameter(Mandatory = $true,HelpMessage='Tenant Client Secret')] [string]$ClientSecret,
            [Parameter(Mandatory = $true,HelpMessage='Tenant Name')] [string]$TenantName,
    ...
    Function Get-Office365Report     {
        ...
        #>
        Param (
            [Parameter(Mandatory=$true,HelpMessage='Client ID')][string]$ClientID,
            [Parameter(Mandatory=$true,HelpMessage='Client Secret')][string]$ClientSecret,
            [Parameter(Mandatory=$true,HelpMessage='Tenant Name')][string]$TenantName,
            [Parameter(Mandatory=$true,HelpMessage='Tenant URL')][string]$Url
        )
        $Dash = '---'
        Try {
            ...
            If ($token.access_token -eq $null) {
                ...
                Write-ScriptLog -LogFile $scriptLog -Content 'Successfully retrieved OAuth access token:',$token
                ### Perform REST call.
    ...
    Function Get-ActiveUserHashTable {
        ...
        #>
        Param (
            [Parameter(Mandatory=$true,HelpMessage='PSCustomObject')][Object]$ReportResults,
            [int]$Months=12
        )
        ...
        Try {
            ...
            #>
            $ELAD            = 'Exchange Last Activity Date'
            $FilteredResults = ($ReportResults | Where-Object {($_.$ELAD -ne '') -and ($_.$ELAD -ne $null) -and ([datetime]($_.$ELAD) -ge (Get-Date).AddMonths($Months))})
            $FilteredResults | ForEach-Object {$HashTable.Add($_.'User Principal Name',$_.$ELAD)}
            $HashTable
        } Catch {
...
Process {
    ...
    If (($ReportResults -eq $false) -or ($ReportResults -eq $null)) {
        ...
        If ($O365ActiveUserHash -is [hashtable]) {
            ...
            $adParams = @{
                Filter      = '(WhenCreated -lt $oldDate) -and ((LastLogonDate -lt $oldDate) -or (LastLogonDate -notlike "*"))'
                SearchBase  = $SearchBase
                SearchScope = 'Subtree'
                Properties  = 'SamAccountName', 'UserPrincipalName', 'LastLogonDate', 'employeeID', 'Enabled', 'Description', 'CanonicalName'
            }
            #$users   = @(Get-ADUser -Filter {(WhenCreated -lt $oldDate) -and ((LastLogonDate -lt $oldDate) -or (LastLogonDate -notlike '*'))} -SearchBase  $SearchBase -SearchScope Subtree -Properties  SamAccountName, UserPrincipalName, LastLogonDate, employeeID, Enabled, Description)
            $users    = @(Get-ADUser @adParams | Where-Object {$_.CanonicalName -notmatch $ignoreOU} | Where-Object {$_.Name -notmatch $IgnoreName} | Where-Object {$_.SamAccountName -notmatch $IgnoreSAM})
            #$Users | ogv
            ...
            ForEach ($user in $users) {
                ### Decide user description
                $Desc = If ($user.Description.Length -gt 0){" - $($user.Description)"} Else {''}
                ...
                If ($O365ActiveUserHash.ContainsKey($user.UserPrincipalName) -eq $false) {
                    ...
                    Try {
                        ...
                        Set-ADUser    -Identity $user.SamAccountName    -Description "Inactive Account, Disabled on $(Get-Date -Format 'yyyy-MM-dd hh:mm:ss tt')$($Desc)"
                        ...
                        $errorUser    = [PSCustomObject][Ordered]@{
            ...
            If ($successUsers.Count -gt 0 -or $errorUsers.Count -gt 0) {
                ...
                If ($errorUsers.Count -gt 0) {
                    ...
                    $emailBody     += "      <strong><font color=""red"">$($errorUsers.Count) User Account$(If ($errorUsers.Count -gt 1){$s}) with problems:</font></strong></br>"
                ...
                If ($successUsers.Count -gt 0) {
                    $emailBody += '    <p>'
                    $emailBody += "      <strong>$($successUsers.Count) User Account$(If($successUsers.Count -gt 1){$s}) Successfully Disabled:</strong></br>"
                ...
                Try {
                    ...
                    $MailCompose.Subject = "Disabled User Accounts - $((Get-Date).ToShortDateString())"
                    $MailCompose.Body    = $emailBody
                    Send-MailMessage @MailCompose

                    #Send-MailMessage -SmtpServer $mailServer -Subject "Disabled User Accounts - $((Get-Date).ToShortDateString())" -From 'scriptcenter@example.edu' -To 'SystemNotices@example.edu' -Cc 'cherylnardone@example.edu' -Body $emailBody -BodyAsHtml:$true
